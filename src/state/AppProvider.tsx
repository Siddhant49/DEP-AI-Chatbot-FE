...
import { ChatHistoryLoadingState, DBHealth, historyList, historyEnsure, DBStatus } from '../api';
import { Conversation } from '../api';
  
export interface AppState {
    isChatHistoryOpen: boolean;
    chatHistoryLoadingState: ChatHistoryLoadingState;
    isDBAvailable: DBHealth;
...
export type Action =
    ...
    | { type: 'SET_DB_STATUS', payload: DBHealth }
...
type AppStateProviderProps = {
  ...
  export const AppStateProvider: React.FC<AppStateProviderProps> = ({ children }) => {
    ...
    useEffect(() => {
        // Check for db config and fetch initial data here
        ...
        const getHistoryEnsure = async () => {
                ...
                if(response?.DB){
                    ...
                    .then((res) => {
                        if(res){
                            ...
                            dispatch({ type: 'SET_DB_STATUS', payload: response });
                        }else{
                            dispatch({ type: 'UPDATE_CHAT_HISTORY_LOADING_STATE', payload: ChatHistoryLoadingState.Fail });
                            dispatch({ type: 'SET_DB_STATUS', payload: {DB: false, status: DBStatus.NotWorking} });
                        }
                    })
                    .catch((err) => {
                        dispatch({ type: 'UPDATE_CHAT_HISTORY_LOADING_STATE', payload: ChatHistoryLoadingState.Fail });
            ...
            .catch((err) => {
                dispatch({ type: 'UPDATE_CHAT_HISTORY_LOADING_STATE', payload: ChatHistoryLoadingState.Fail });
                dispatch({ type: 'SET_DB_STATUS', payload: {DB: false, status: DBStatus.NotConfigured} });
            })
        }
        getHistoryEnsure();
    }, []);
