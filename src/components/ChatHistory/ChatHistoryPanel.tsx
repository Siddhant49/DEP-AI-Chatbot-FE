...
import { setResponsiveItem } from "../../pages/chat/Chat";
...
export function ChatHistoryPanel(props: ChatHistoryPanelProps) {
    ...
    const handleHistoryClick = () => {
        ...
        setResponsiveItem('isChatHistory', 'false');
    };
    ...
    const onClearAllChatHistory = async () => {
        ...
        if(!response.ok){
            ...
            sessionStorage.setItem('isCitationPannel', 'false');
        ...
        <section className={styles.container} data-is-scrollable aria-label={"chat history panel"}>
                ...
                <Stack verticalAlign="start">
                    <Stack horizontal styles={commandBarButtonStyle}>
                        <CommandBarButton
                            ...
                            disabled={!appStateContext?.state.chatHistory || appStateContext?.state.chatHistory.length === 0}
            ...
            <Stack aria-label="chat history panel content"
                ...
                <Stack className={styles.chatHistoryListContainer}>
                    {(appStateContext?.state.chatHistoryLoadingState === ChatHistoryLoadingState.Success && appStateContext?.state.isDBAvailable.DB) && <ChatHistoryList/>}
                    {(appStateContext?.state.chatHistoryLoadingState === ChatHistoryLoadingState.Fail && appStateContext?.state.isDBAvailable) && <>
                        <Stack>
                            <Stack horizontalAlign='center' verticalAlign='center' style={{ width: "100%", marginTop: 10 }}>
                                <StackItem>
                                    <Text style={{ alignSelf: 'center', fontWeight: '400', fontSize: 16 }}>
                                        {appStateContext?.state.isDBAvailable?.status && <span>{appStateContext?.state.isDBAvailable?.status}</span>}
                                        {!appStateContext?.state.isDBAvailable?.status && <span>Error loading chat history</span>}
